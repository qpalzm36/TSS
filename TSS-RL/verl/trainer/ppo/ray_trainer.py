    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if 'sub_scores' in batch.non_tensor_batch:
                    avg_sub_scores = {}
                    sub_scores_list = batch.non_tensor_batch['sub_scores']
                    
                    # 使用 len() 代替 if sub_scores_list (因为它是 numpy 数组)
                    if sub_scores_list is not None and len(sub_scores_list) > 0:
                        first_dict = sub_scores_list[0]
                        if isinstance(first_dict, dict):
                            for skey in first_dict.keys():
                                # 取得 Batch 中该项分数的平均值
                                scores = [d.get(skey, 0.0) for d in sub_scores_list if isinstance(d, dict)]
                                if scores:
                                    avg_sub_scores[skey] = np.mean(scores)
                    
                    metrics.update(avg_sub_scores)
