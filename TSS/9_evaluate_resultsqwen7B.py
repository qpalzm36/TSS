...
def main(args):
    ...
    for result in tqdm(all_results, desc=""):
        ...
        for i in range(comparison_length):
            ...
            if not eval_res_graded or "score" not in eval_res_graded:
                
                total_skipped_step_accuracy += 1
                problem_skipped_count += 1
                detail["step_accuracy_graded"] = "skipped (evaluation failed)"
            else:
        ...
        problem_eval["scores"]["skipped_step_accuracy_count"] = problem_skipped_count
    ...
    report["total_skipped_step_accuracy_evaluations"] = total_skipped_step_accuracy
    report["per_problem_skipped_details"] = [
        {
            "problem_id": item["problem_id"],
            "skipped_count": item["scores"]["skipped_step_accuracy_count"]
        } for item in full_evaluation_data if item["scores"]["skipped_step_accuracy_count"] > 0
    ]
